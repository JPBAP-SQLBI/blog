...
※ データフロー（Gen1）およびデータマートはサポートされていないため、VNet データゲートウェイを利用しデータソースからデータを取得することはできません。
一方、後述のリンクの通り、例えばセマンティックモデルで VNet データゲートウェイを利用し、データソースとしてデータフロー（Gen1）やデータマートのデータを取得することは可能です。
