...
#### データポイント
Power BIのグラフやテーブルはインタラクティブに動作するもので、例えばグラフやテーブル上の数字やデータポイントにカーソルを合わせると、詳細なデータが表示されます。
...
営業成績やKPIなどの数字を毎日Excelファイルの日報として受け取る代わりに、Power BI上でレポートを作成して受信登録をご利用いただけます。
